...
class SqlPatternSelectNum(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VER) {
    companion object {
        private const val DATABASE_VER  = 2
        private const val DATABASE_NAME = "PATTERNSELECTNUM.db"

        private const val TABLE_NAME = "PatSelNum"
    ...
    fun addPatSel(aLottoNum: PatSelNum) {
        val sDb = this.writableDatabase
    ...
    fun updatePatSel(aLottoNum: PatSelNum):Int {
        val sDb = this.writableDatabase
        val sVal = ContentValues()
    ...
    fun deletePatSel(aLottoNum: PatSelNum) {
        val sDb = this.writableDatabase
